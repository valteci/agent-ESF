#!/usr/bin/env python3
"""
Validador do JSON gerado pela skill `crypto-investment-json-extractor`.
...
- Cada objeto de transação deve conter exatamente as chaves:
...
def validate_transaction(transaction: Any, index: int) -> list[str]:
    ...
    Args:
        ...
        index: Índice da transação dentro da lista.
...
def validate_payload(payload: Any) -> list[str]:
    ...
    Args:
        payload: JSON já desserializado em objeto Python.

    Returns:
        Lista de erros encontrados. Lista vazia significa payload válido.
    """
    errors: list[str] = []
...
def validate_json_text(json_text: str) -> list[str]:
    ...
    Args:
        json_text: Conteúdo JSON em string.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Valida o JSON gerado pela skill crypto-investment-json-extractor."
    )
    parser.add_argument(
        "file",
        nargs="?",
        help="Caminho para o arquivo JSON. Se omitido, lê do stdin.",
    )
